      ...
      <div className="export-workspace-card">
        ...
        <div className="export-format-tabs" role="tablist" aria-label="Export format">
          {(Object.keys(formatCopy) as ExportFormat[]).map((key) => {
            ...
            return <button type="button" role="tab" aria-selected={format === key} className={format === key ? "active" : ""} key={key} onClick={() => setFormat(key)}><ItemIcon size={20} /><span>{key === "png" ? "PNG sequence" : key.toUpperCase()}</span></button>;
          })}
        </div>
        <section className="export-format-panel">
          ...
          <div className="export-dimensions">
            <label><span>Width</span><input type="number" min="64" max="7680" step="2" value={props.width} onChange={(event) => props.onWidthChange(Number(event.target.value))} /><small>px</small></label>
            <i>×</i>
            <label><span>Height</span><input type="number" min="64" max="4320" step="2" value={props.height} onChange={(event) => props.onHeightChange(Number(event.target.value))} /><small>px</small></label>
            <label><span>FPS</span><input type="number" min="1" max="120" step="1" value={props.fps} onChange={(event) => props.onFpsChange(Number(event.target.value))} /><small>fps</small></label>
          </div>
          <div className="export-edit-controls">
            <label><span>Trim in</span><input type="number" min="0" max={props.durationMs / 1000} step="0.01" value={(props.trimStartMs / 1000).toFixed(2)} onChange={(event) => props.onTrimStartChange(Number(event.target.value) * 1000)} /><small>sec</small></label>
            <label><span>Trim out</span><input type="number" min="0" max={props.durationMs / 1000} step="0.01" value={(props.trimEndMs / 1000).toFixed(2)} onChange={(event) => props.onTrimEndChange(Number(event.target.value) * 1000)} /><small>sec</small></label>
            <label><span>Playback speed</span><select value={props.speed} onChange={(event) => props.onSpeedChange(Number(event.target.value))}><option value="0.25">0.25×</option><option value="0.5">0.5×</option><option value="1">1×</option><option value="1.5">1.5×</option><option value="2">2×</option><option value="4">4×</option></select></label>
            <p>Edits are non-destructive. Every export samples interpolated motion from the original take.</p>
          </div>
          <div className="export-summary-grid">
            ...
            <span><small>EDITED DURATION</small><strong>{(editedDurationMs / 1000).toFixed(2)} seconds</strong></span>
            <span><small>{format === "png" ? "ZIP CONTENTS" : "OUTPUT"}</small><strong>{format === "png" ? `${sequenceFrames.toLocaleString()} numbered PNGs` : `${props.width} × ${props.height} · ${props.fps} FPS`}</strong></span>
          </div>
          ...
          <button type="button" className="primary-button export-primary" disabled={props.busy || (!props.hasTake && !props.hasVideo)} onClick={exportAction}>
